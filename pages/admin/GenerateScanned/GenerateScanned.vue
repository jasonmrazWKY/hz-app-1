					    ...
					    <view class="uni-list-cell">
					        <view class="uni-triplex-row">
					            <view class="uni-triplex-left" style="width: 100%;">
					                <text class="uni-title uni-ellipsis">员工:{{item1.uname}}<text class="kg"></text>编号:{{item1.id}}</text>
					                <text class="uni-text">扫描时间:{{item1.inputtime}}<text class="kg"></text>床/缸号:{{item1.chuanghao}}</text>
					                <text class="uni-text">货号:{{item1.huohao}}<text class="kg"></text>颜色:{{item1.color}}<text class="kg"></text>尺码:{{item1.chima}}<text class="kg"></text></text>
									<text class="uni-text-small uni-ellipsis" style="color: #FFAC60;font-size: 32upx;">工序:{{item1.worktype}}<text class="kg"></text>数量:{{item1.sl}}<text class="kg"></text>工价:{{item1.price.toFixed(3)}}<text class="kg"></text>金额:{{item1.je.toFixed(2)}}</text>
					            </view>
					        </view>
					    </view>
					</block>
	...
	export default {
		...
		methods: {
			initDate(){
				setTimeout(() => {
					uni.request({
						url:`https://${this.dk}/getSearchYsk.aspx`,
						method: 'GET',
						data: {
							dates:this.dates,
							datee:this.datee,
