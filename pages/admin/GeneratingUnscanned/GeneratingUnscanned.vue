<template>
	<view class="content">
		...
		<view class="m-top1" style="height: 64upx;"></view>
		<view class="warp">
					    ...
					    <view class="uni-list-cell">
					        <view class="uni-triplex-row">
					            <view class="uni-triplex-left" style="width: 100%;">
					                <text class="uni-title uni-ellipsis">编号:{{item1.id}}<text class="kg"></text>床/缸号:{{item1.chuanghao}}<text class="kg"></text>货号:{{item1.huohao}}</text>
					                <text class="uni-text">颜色:{{item1.color}}<text class="kg"></text>尺码:{{item1.chima}}<text class="kg"></text>工序:{{item1.worktype}}<text class="kg"></text>数量:{{item1.sl}}</text>
					            </view>
					        </view>
					    </view>
					</block>
	...
	export default {
		...
		methods: {
			initDate(){
				setTimeout(() => {
					uni.request({
						url:`https://${this.dk}/getSearchWsk.aspx`,
						method: 'GET',
						data: {
							dates:this.dates,
							datee:this.datee,
...
/* .hj{color: red;font-weight: 600;font-size: 36upx;} */
